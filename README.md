...
| `install.sh upgrade` | скачать свежие конфиги, правила и дашборды и пересоздать стек |
| `install.sh update` | только образы: `docker compose pull` + `up -d` |

`upgrade` заменяет `central/` и `agent/` целиком, сохраняя `.env`, и пересоздаёт контейнеры —
иначе изменившийся конфиг агента остался бы старым в томе. Ручные правки в конфигах он затрёт;
если в новой версии появились переменные, которых нет в твоём `.env`, он их перечислит.
